$(document).ready(women couture.html){

    var=background,foreground1,header1,p1,navBar,select box,foregroundVideo,searchBar,logoImage,toggleSlider,shoppingNavCart,bluePill,parallex,emP,costOfItems,coutureImages,addToCart;
    ...
    function cartclick(){let button=this;button.classList.add('clicked');}  
            ...
            costOfItems=$(costOfItems).isNumeric();
            addToCart=$(adToCart).each().class(fas-fa hopping cart plus)on(.click slideDown,function(e){e.target||e.scrElement;
